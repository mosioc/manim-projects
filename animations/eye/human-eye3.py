...
class HumanEyeAnatomy3D(ThreeDScene):
    def construct(self):
        ...
        # Create the basic 3D eye structure
        eye_center = ORIGIN
        eye_radius = 2.5
        
        # Use Sphere for 3D eye outline
        eye_outline = Sphere(radius=eye_radius, color="#073642").move_to(eye_center)
        
        # Define positions relative to eye center (including z-axis)
        front_of_eye = eye_center + LEFT * eye_radius * 0.7
        back_of_eye = eye_center + RIGHT * eye_radius * 0.7
        
        # Inner structures (3D)
        iris = Sphere(radius=0.8, color="#268BD2", fill_opacity=0.8).move_to(front_of_eye)
        cornea = Arc(radius=1.1, angle=PI/2, color="#839496", stroke_width=3).rotate(PI).move_to(front_of_eye + UP * 0.2)
        lens = Ellipse(width=0.6, height=1.0, color="#839496", fill_opacity=0.3).move_to(front_of_eye + RIGHT * 0.5)
        retina = Arc(radius=eye_radius-0.2, angle=TAU * 0.7, color="#B58900", fill_opacity=0.6).move_to(eye_center).shift(RIGHT * 0.2)
        
        # Optic nerve - Use Cylinder for 3D
        optic_nerve = Cylinder(radius=0.2, height=1.5, color="#6C4F47", fill_opacity=0.7).move_to(eye_center + RIGHT * eye_radius + RIGHT * 0.2)
        ...
        def create_label(text, target, direction, offset=ORIGIN, line_offset=ORIGIN):
            label = Text(text, font_size=24, color="#586E75")
        ...
        iris_label = create_label("Iris", iris, LEFT, LEFT*0.2, UP*0.2)
        ...
        optic_nerve_label = create_label("Optic nerve", optic_nerve, UP, UP*0.3)
        
        labels.add(cornea_label, iris_label, lens_label, retina_label, optic_nerve_label)
        
        # Animation sequence with pulse effect
        ...
        # Add the growing optic nerve effect
        self.play(ScaleInPlace(optic_nerve, scale_factor=0.1))
