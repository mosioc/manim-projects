    ...
    def construct(self):
        ...
        for k in range(2,52):
            ...
            Circ=ParametricFunction(
                lambda u: (3*math.cos(u),3*math.sin(u),0), t_range=[0,2*PI])
        ...
        for x in DiffDiagrams:
            anis1.append(Create(x[0]))#ShowCreation(x[0]))
        self.play(*anis1, run_time=3)
 
        anis2 = []
        for x in DiffDiagrams:
            anis2.append(Create(x[1]))#ShowCreation(x[1]))
        self.play(*anis2, run_time=1)#10)
        self.wait(2)
        ...
        for j in thelist:
            ...
            self.play(Create(DiffDiagrams[j][1]), run_time=3)#ShowCreation(DiffDiagrams[j][1]), run_time=3)
            i += 1
        self.wait(5)